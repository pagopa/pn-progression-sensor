...
describe("event mapper tests", function () {
  ...
  it("test REFINEMENT rework attempt 0 without old attempt 1", async () => {
      ...
      ddbMock.on(BatchGetCommand).resolves(batchGet);
      ddbMock.on(QueryCommand).resolves({
        Items: [{iun: 'abcd', timestamp: '2022-05-02T00:00:00Z', timelineElementId: 'NOTIFICATION_TIMELINE_REWORKED.IUN_abcd.RECINDEX_0.ATTEMPT_0.REWORK_0', details: {recIndex:0, sentAttemptMade:0, invalidatedTimelineAndStatusHistory:[{
  ...
  it("test NOTIFICATION_TIMELINE_REWORKED", async () => {
    ...
    event.dynamodb.NewImage.timelineElementId = { S: "NOTIFICATION_TIMELINE_REWORKED.IUN_IUN1.RECINDEX_0.ATTEMPT_0.REWORK_0" };
